...
def player_turn():
    ...
    while True:
        ...
        if len(coordinates) != 2:
            ...
            continue
        elif coordinates[0] not in '123' or coordinates[1] not in '123':
            print('Введите координаты от 1 до 3 включительно: ', end='')
            continue
        elif area[int(coordinates[0]) - 1][int(coordinates[1]) - 1] != '-':
            print('Повторение хода. Введите другие координаты: ', end='')
            continue
        else:
            break
    return list(int(i) - 1 for i in coordinates)
